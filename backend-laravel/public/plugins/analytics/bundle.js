/**
 * Auto-generated stub bundle for module: analytics
 * This registers sidebar items and routes via the hook system.
 * Replace with a full bundle when custom UI components are available.
 */
(function() {
  var id = 'analytics';
  var hooks = (window.__APP_BRIDGE__ && window.__APP_BRIDGE__.hooks) || window.__APP_HOOKS__;
  if (!hooks) { console.warn('[Plugin:' + id + '] Hooks not available'); return; }

  // Register sidebar items
  hooks.addFilter('sidebar_items', function(items) {
    items.push({key:'analytics/dashboard',label:'Thống kê',icon:'BarChart3',featureGroup:'store',moduleId:'analytics'});
    return items;
  });

  // Register admin routes
  hooks.addFilter('admin_routes', function(config) {
    Object.assign(config.routeToTab, {'analytics/dashboard':'analytics-dashboard'});
    return config;
  });

  // Register plugin
  var plugin = {
    id: id,
    name: 'Phân tích & Thống kê',
    version: '1.0.0',
    sidebar: {"group":"Báo cáo","items":[{"key":"analytics-dashboard","label":"Thống kê","icon":"BarChart3","route":"analytics\/dashboard"}]}
  };
  window.__PLUGIN_REGISTRY__ = window.__PLUGIN_REGISTRY__ || {};
  window.__PLUGIN_REGISTRY__[id] = plugin;
})();

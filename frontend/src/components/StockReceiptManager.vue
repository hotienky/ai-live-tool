          ...
          <tr v-for="r in receipts" :key="r.id">
            ...
            <td>
              <div class="action-btns">
                ...
                <button v-if="r.status !== 'cancelled'" class="act-btn act-cancel" @click="cancelReceipt(r)"><X :size="13" /> {{ t('admin.msg_1e405035', 'Hủy') }}</button>
...
function addProduct(p) {
  ...
  if (form.value.items.find(i => i.product_id === p.id)) {
    showToast(t('admin.msg_1fc45d', 'Sản phẩm đã có trong danh sách'), 'warning')
    return
  }
...
.sr-header h2 { margin: 0; font-size: 20px; font-weight: 800; display: flex; align-items: center; gap: 10px; }

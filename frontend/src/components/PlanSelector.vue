<template>
  <div class="space-y-6">
    <div class="text-center mb-8">
      <h2 class="text-2xl font-bold">Chọn Gói Cước Phù Hợp</h2>
      <p class="text-muted-foreground mt-2">Nâng cấp để mở khóa thêm tài nguyên và tính năng cho website của bạn.</p>
    </div>
      ...
      >
        ...
        <div class="mb-4">
          ...
          <div class="mt-2 flex items-baseline text-3xl font-extrabold">
            ...
            <span class="text-sm font-medium text-muted-foreground ml-1">/{{ plan.billing_cycle === 'yearly' ? 'năm' : 'tháng' }}</span>
          </div>
        </div>
        ...
        <ul class="space-y-3 mb-6 flex-1 text-sm">
          <li class="flex items-center">
            ...
            <span>Tối đa <strong>{{ plan.limits?.pages || 'Không giới hạn' }}</strong> trang CMS</span>
          </li>
          <li class="flex items-center">
            <Check class="h-4 w-4 text-green-500 mr-2 flex-shrink-0" />
            <span><strong>{{ plan.limits?.storage_mb || 0 }} MB</strong> dung lượng lưu trữ</span>
          </li>
          <li class="flex items-center">
            <Check class="h-4 w-4 text-green-500 mr-2 flex-shrink-0" />
            <span>Cài đặt <strong>{{ plan.limits?.modules_free || 0 }}</strong> module chức năng</span>
          </li>
          <li class="flex items-center">
            <Check class="h-4 w-4 text-green-500 mr-2 flex-shrink-0" />
            <span><strong>{{ plan.limits?.custom_domains || 0 }}</strong> tên miền riêng</span>
          </li>
          <li v-if="plan.limits?.headless_mode" class="flex items-center">
            <Check class="h-4 w-4 text-green-500 mr-2 flex-shrink-0" />
            <span>Hỗ trợ API Headless mode</span>
          </li>
        </ul>
        ...
        >
          ...
          {{ currentPlanId === plan.id ? 'Đang Sử Dụng' : 'Chọn Gói Này' }}
        </button>
      </div>
    </div>
  </div>
...
const fetchPlans = async () => {
  try {
    ...
    toast.error('Lỗi khi tải danh sách gói cước')
    console.error(error)
  } finally {
    loading.value = false
  }
...
const selectPlan = async (plan) => {
  ...
  try {
    ...
    toast.error('Lỗi khi thay đổi gói cước. Vui lòng thử lại.')
    console.error(error)
  } finally {

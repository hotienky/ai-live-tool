  ...
  <div class="lead-panel">
    <div class="lead-panel__header">
      <h2 class="lead-panel__title">
        <span class="lead-panel__title-badge"><DollarSign :size="18" /></span>
        {{ t('admin.potential_customers', 'Khách hàng tiềm năng') }}
        <span class="lead-panel__count" v-if="filteredLeads.length">{{ filteredLeads.length }}</span>
      </h2>
      ...
      <div class="lead-panel__search">
        ...
        />
        <button v-if="searchQuery" class="lead-panel__search-clear" @click="searchQuery = ''">
          <X :size="14" />
        </button>
      </div>
      ...
      <div class="lead-panel__tags" v-if="dynamicKeywords.length > 0">
        ...
        <button v-if="activeKeyword" class="lead-panel__tag lead-panel__tag--clear" @click="activeKeyword = ''">
          <X :size="10" /> Bỏ lọc
        </button>
      </div>
    </div>
    ...
    <div class="lead-panel__list" ref="listRef">
      <div v-if="filteredLeads.length === 0" class="lead-panel__empty">
        <div class="lead-panel__empty-icon"><RadioTower :size="48" /></div>
        ...
        <p class="lead-panel__empty-sub">
          ...
          <template v-else>Các khách hàng tiềm năng sẽ hiện ở đây</template>
        </p>
      </div>
      <LeadCard
...
const STOP_WORDS = new Set(['của','và','là','có','cho','với','được','các','từ','trong','này','đó','những','một','không','cũng','như','thì','mà','khi','ở','đã','sẽ','đang','bị','vì','nên','hay','hoặc','nhưng','nếu','vậy','rồi','lại','còn','em','anh','chị','mình','ơi','nhé','nha','ạ','thế','quá','rất','lắm','gì','nào','đâu','sao','bao','mấy','tôi','bạn','bé','con','cái','đi','về','ra','vào','lên','xuống','hỏi','xin','dạ','vâng','ok','ko','k','dc','đc','mn','ah','à','uh','hả','hen','the','and','a','an','of','to','in','is','it','for','on','with','at','by','be','this'])
...
function extractKeywords(text) {
  ...
  for (const w of words) { if (w.length >= 3 && !/^\d+$/.test(w)) keywords.add(w) }
...
const dynamicKeywords = computed(() => {
  ...
  for (const lead of props.leads) {
    for (const kw of extractKeywords(lead.comment)) { freq[kw] = (freq[kw] || 0) + 1 }
  }
  return Object.entries(freq).sort((a, b) => b[1] - a[1]).map(([kw]) => kw)
})
...
.lead-panel__title { font-size: 18px; font-weight: 700; margin-bottom: 10px; display: flex; align-items: center; gap: 10px; }
.lead-panel__title-badge { width: 36px; height: 36px; border-radius: 10px; display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, rgba(255,59,92,0.15), rgba(255,140,66,0.15)); color: var(--color-accent-warm); }
.lead-panel__count { background: var(--color-accent-hot); color: white; font-size: 12px; padding: 2px 8px; border-radius: 12px; font-weight: 600; }
.lead-panel__search { display: flex; align-items: center; gap: 8px; padding: 8px 12px; border-radius: 10px; border: 1px solid var(--color-border); background: var(--color-bg-primary); margin-bottom: 10px; transition: all 0.2s; }
...
.lead-panel__search-input { flex: 1; border: none; background: transparent; color: var(--color-text-primary); font-size: 13px; outline: none; }
.lead-panel__search-input::placeholder { color: var(--color-text-muted); }
.lead-panel__search-clear { background: none; border: none; color: var(--color-text-muted); cursor: pointer; padding: 0 4px; display: flex; align-items: center; }
.lead-panel__search-clear:hover { color: var(--color-text-primary); }
.lead-panel__filters { display: flex; gap: 6px; margin-bottom: 8px; }
.lead-panel__tab { display: flex; align-items: center; gap: 4px; padding: 6px 14px; border-radius: 8px; border: 1px solid var(--color-border); background: transparent; color: var(--color-text-secondary); font-size: 13px; cursor: pointer; transition: all 0.2s; }
.lead-panel__tab:hover { background: var(--color-bg-card-hover); color: var(--color-text-primary); }
.lead-panel__tab--active { background: var(--color-bg-card); color: var(--color-text-primary); border-color: var(--color-accent-hot); box-shadow: 0 0 8px rgba(255,59,92,0.1); }
.lead-panel__tags { display: flex; flex-wrap: wrap; gap: 5px; align-items: center; max-height: 60px; overflow-y: auto; }
.lead-panel__tags-label { color: var(--color-text-muted); flex-shrink: 0; }
.lead-panel__tag { display: flex; align-items: center; gap: 3px; padding: 3px 10px; border-radius: 14px; border: 1px solid var(--color-border); background: var(--color-bg-primary); color: var(--color-text-secondary); font-size: 11px; cursor: pointer; transition: all 0.2s; white-space: nowrap; }
...
.lead-panel__empty { display: flex; flex-direction: column; align-items: center; justify-content: center; height: 300px; color: var(--color-text-muted); text-align: center; }
.lead-panel__empty-icon { margin-bottom: 16px; color: var(--color-text-muted); }

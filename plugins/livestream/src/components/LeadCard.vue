<template>
  ...
  >
    ...
    <div class="lead-card__actions">
      ...
      <button class="lead-card__reply-btn" @click.stop="onSuggestReply" :disabled="loadingReply">
    ...
    <div class="lead-card__reply" v-if="suggestedReply">
      <div class="lead-card__reply-header">
        ...
        <button class="lead-card__copy-btn" @click.stop="copyReply">
...
import { apiFetch, useI18n } from '../helpers.js'
import { Flame, CircleDot, ExternalLink, MessageCircle, Loader2, Bot, Copy, Check, ShoppingBag } from 'lucide-vue-next'
...
<style scoped>
.lead-card { background: var(--color-bg-card); border-radius: 12px; padding: 16px; margin-bottom: 12px; border-left: 4px solid transparent; position: relative; animation: fadeSlideIn 0.4s ease-out; transition: transform 0.2s, background 0.2s; }
.lead-card:hover { transform: translateX(4px); background: var(--color-bg-card-hover); }
.lead-card--hot { border-left-color: var(--color-accent-hot); }
.lead-card--warm { border-left-color: var(--color-accent-warm); }
.lead-card__badge { position: absolute; top: 12px; right: 12px; padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 700; letter-spacing: 0.5px; display: flex; align-items: center; gap: 4px; }
...
.lead-card__product { display: flex; align-items: center; gap: 6px; padding: 6px 10px; margin-bottom: 10px; border-radius: 6px; background: rgba(168,85,247,0.08); border: 1px solid rgba(168,85,247,0.2); font-size: 12px; color: #a855f7; }
...
.lead-card__cta { display: inline-flex; align-items: center; gap: 6px; padding: 8px 18px; border-radius: 8px; font-size: 13px; font-weight: 600; text-decoration: none; transition: all 0.2s; cursor: pointer; }
.lead-card__cta--hot { background: var(--color-accent-hot); color: white; }
.lead-card__cta--hot:hover { background: #e63350; transform: scale(1.03); }
.lead-card__cta--warm { background: var(--color-accent-warm); color: white; }
.lead-card__cta--warm:hover { background: #e67a35; transform: scale(1.03); }
.lead-card__reply-btn { display: flex; align-items: center; gap: 5px; padding: 8px 14px; border-radius: 8px; border: 1px solid var(--color-border); background: var(--color-bg-primary); color: var(--color-text-secondary); font-size: 12px; font-weight: 600; cursor: pointer; transition: all 0.2s; }
...
.lead-card__reply { margin-top: 10px; padding: 10px 12px; background: rgba(56,189,248,0.08); border: 1px solid rgba(56,189,248,0.2); border-radius: 8px; }
.lead-card__reply-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px; font-size: 11px; color: #38bdf8; font-weight: 600; }
.lead-card__reply-header span { display: flex; align-items: center; gap: 4px; }
.lead-card__copy-btn { display: flex; align-items: center; gap: 4px; background: none; border: 1px solid rgba(56,189,248,0.3); color: #38bdf8; padding: 2px 10px; border-radius: 6px; font-size: 11px; cursor: pointer; transition: all 0.2s; }

...
.lsm__header { display: flex; justify-content: space-between; align-items: center; padding: 18px 20px 12px; }
.lsm__header h3 { font-size: 16px; font-weight: 700; margin: 0; }
.lsm__close { background: none; border: none; color: var(--color-text-muted, #888); font-size: 18px; cursor: pointer; padding: 4px; }
.lsm__close:hover { color: var(--color-text-primary, #fff); }
.lsm__tabs { display: flex; gap: 4px; padding: 0 16px 12px; }
.lsm__tab {
...
.lsm__label { display: block; font-size: 12px; font-weight: 600; color: var(--color-text-secondary, #aaa); margin-bottom: 4px; margin-top: 12px; }
...
.lsm__actions { display: flex; gap: 8px; margin-top: 16px; }
.lsm__btn {
...
.lsm__btn--live { background: linear-gradient(135deg, #ff3b5c, #e63350); color: white; flex: 2; box-shadow: 0 4px 16px rgba(255,59,92,0.3); }
.lsm__btn--live:hover:not(:disabled) { background: linear-gradient(135deg, #e63350, #cc2a44); transform: translateY(-1px); }
.lsm__btn--mock { background: var(--color-bg-primary, #10101e); color: var(--color-text-secondary, #aaa); border: 1px solid var(--color-border, #2a2a4a); }
.lsm__btn--mock:hover:not(:disabled) { color: var(--color-text-primary, #fff); border-color: var(--color-text-muted, #666); }
.lsm__spinner { width: 14px; height: 14px; border: 2px solid rgba(255,255,255,0.3); border-top-color: white; border-radius: 50%; animation: spin 0.6s linear infinite; }
@keyframes spin { to { transform: rotate(360deg); } }
.lsm__recent { border-top: 1px solid var(--color-border, #2a2a4a); padding: 12px 20px 16px; }
.lsm__recent-title { font-size: 11px; font-weight: 700; text-transform: uppercase; color: var(--color-text-muted, #666); margin-bottom: 8px; letter-spacing: 0.5px; }
.lsm__recent-item { display: flex; align-items: center; gap: 8px; padding: 7px 10px; border-radius: 6px; cursor: pointer; transition: background 0.15s; }

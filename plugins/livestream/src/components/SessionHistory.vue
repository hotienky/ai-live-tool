    ...
    <div class="session-history__panel">
      ...
      <div class="session-history__body">
        <div v-if="loading" class="session-history__loading">
          <Loader2 :size="20" class="session-history__spin" />{{ t('admin.msg_d5fe42f6', 'Đang tải...') }}
        </div>
        ...
        <div v-else class="session-history__list">
          ...
          >
            ...
            <div class="session-card__meta">
              <span><Calendar :size="11" />{{ formatDate(session.started_at) }}</span>
...
import { apiFetch, logger, useI18n } from '../helpers.js'
import {
...
<style scoped>
.session-history__overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.5); z-index: 998; }
.session-history__panel { position: fixed; top: 0; right: 0; width: 400px; max-width: 90vw; height: 100vh; background: var(--color-bg-secondary); border-left: 1px solid var(--color-border); z-index: 999; display: flex; flex-direction: column; animation: slideInRight 0.3s ease-out; }
@keyframes slideInRight { from { transform: translateX(100%); } to { transform: translateX(0); } }
.session-history__header { display: flex; justify-content: space-between; align-items: center; padding: 16px 20px; border-bottom: 1px solid var(--color-border); flex-shrink: 0; }
.session-history__title { display: flex; align-items: center; gap: 8px; font-size: 16px; font-weight: 700; }
.session-history__close { background: none; border: none; color: var(--color-text-muted); cursor: pointer; padding: 4px; border-radius: 6px; transition: all 0.2s; }
.session-history__close:hover { color: var(--color-text-primary); background: var(--color-bg-card); }
.session-history__body { flex: 1; overflow-y: auto; padding: 16px; }
.session-history__loading { display: flex; align-items: center; justify-content: center; gap: 8px; padding: 40px; color: var(--color-text-muted); font-size: 14px; }
.session-history__spin { animation: spin 1s linear infinite; }
@keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
.session-history__empty { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 8px; padding: 60px 20px; color: var(--color-text-muted); text-align: center; }
.session-history__empty-sub { font-size: 13px; }
.session-history__list { display: flex; flex-direction: column; gap: 10px; }
.session-card { background: var(--color-bg-card); border-radius: 10px; padding: 14px; position: relative; transition: transform 0.2s, background 0.2s; border-left: 3px solid var(--color-border); }
...
.session-card__top { display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; }
.session-card__platform { display: flex; align-items: center; gap: 6px; font-weight: 600; font-size: 13px; color: var(--color-text-primary); }
.session-card__badge { font-size: 11px; padding: 2px 10px; border-radius: 12px; font-weight: 600; }
.session-card__badge--active { background: rgba(16,185,129,0.15); color: #10b981; }
.session-card__badge--ended { background: rgba(107,114,128,0.15); color: #9ca3af; }
.session-card__stats { display: flex; gap: 14px; margin-bottom: 8px; }
.session-card__stat { display: flex; align-items: center; gap: 4px; font-size: 12px; color: var(--color-text-secondary); }
.session-card__stat-icon--hot { color: var(--color-accent-hot); }
.session-card__stat-icon--warm { color: var(--color-accent-warm); }
.session-card__meta { display: flex; gap: 12px; font-size: 11px; color: var(--color-text-muted); }
.session-card__meta span { display: flex; align-items: center; gap: 4px; }
.session-card__live-dot { color: #10b981; animation: dotPulse 1.5s infinite; }
.session-card__delete { position: absolute; top: 12px; right: 12px; background: none; border: none; color: var(--color-text-muted); cursor: pointer; padding: 4px; border-radius: 4px; opacity: 0; transition: all 0.2s; }

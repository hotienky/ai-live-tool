<template>
  <div class="inv-report">
    ...
    <div v-if="activeTab === 'stock'" class="tab-panel">
      <div class="rpt-stats">
        <div class="stat-card"><div class="stat-icon"><Package :size="20" /></div><div class="stat-value">{{ stockSummary.total_products || 0 }}</div><div class="stat-label">{{ t('admin.promotion.product', 'Sản phẩm') }}</div></div>
      ...
      <table>
        <thead><tr><th>{{ t('admin.promotion.product', 'Sản phẩm') }}</th><th>SKU</th><th>{{ t('admin.stock', 'Tồn kho') }}</th><th>{{ t('admin.msg_8d0583c6', 'Tối thiểu') }}</th><th>{{ t('admin.msg_66a3ea84', 'Giá vốn') }}</th><th>{{ t('admin.selling_price', 'Giá bán') }}</th><th>{{ t('admin.msg_ba2a45b1', 'Giá trị kho') }}</th><th>TT</th></tr></thead>
        <tbody>
    ...
    <div v-if="activeTab === 'movement'" class="tab-panel">
      ...
      <table>
        <thead><tr><th>{{ t('admin.promotion.product', 'Sản phẩm') }}</th><th>SKU</th><th>{{ t('admin.msg_fea5d4f7', 'Đầu kỳ') }}</th><th>{{ t('admin.msg_9b9d9310', 'Nhập') }}</th><th>{{ t('admin.msg_296fd4c2', 'GT nhập') }}</th><th>{{ t('admin.msg_e1339200', 'Xuất') }}</th><th>{{ t('admin.msg_4bbd5c52', 'GT xuất') }}</th><th>{{ t('admin.msg_160547e0', 'Cuối kỳ') }}</th></tr></thead>
        <tbody>
    ...
    <div v-if="activeTab === 'cogs'" class="tab-panel">
      ...
      <table>
        <thead><tr><th>{{ t('admin.msg_b9474a12', 'Ngày') }}</th><th>{{ t('admin.msg_e9c02d54', 'Mô tả') }}</th><th>{{ t('admin.orders', 'Đơn hàng') }}</th><th>{{ t('admin.msg_66a3ea84', 'Giá vốn') }}</th></tr></thead>
        <tbody>
    ...
    <div v-if="activeTab === 'alerts'" class="tab-panel">
      ...
      <table>
        <thead><tr><th>{{ t('admin.promotion.product', 'Sản phẩm') }}</th><th>SKU</th><th>{{ t('admin.stock', 'Tồn kho') }}</th><th>{{ t('admin.msg_8d0583c6', 'Tối thiểu') }}</th><th>{{ t('admin.status', 'Trạng thái') }}</th></tr></thead>
        <tbody>
...
const tabs = [
  { key: 'stock', label: t('admin.stock', 'Tồn kho'), icon: Package },
  ...
  { key: 'alerts', label: t('admin.warning', 'Cảnh báo'), icon: AlertTriangle },
]
